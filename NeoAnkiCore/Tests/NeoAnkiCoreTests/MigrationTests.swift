...
@Test func freshDatabaseCreatesCurrentSchema() async throws {
    ...
    #expect(try tableExists("resource_revisions", at: url))
    #expect(try tableExists("api_changes", at: url))
    ...
    #expect(try tableExists("scheduler_cohorts", at: url))
    #expect(try tableExists("scheduler_card_history", at: url))
    #expect(try columnExists("cohort_id", in: "fsrs_parameter_sets", at: url))
    #expect(try columnExists("cohort_id", in: "fsrs_optimization_runs", at: url))
