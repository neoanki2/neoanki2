...
/// - New/learning/relearning + Again: update D/S with FSRS and remain due now
///   for another repair round in the current session.
...
/// still updates FSRS memory state so the eventual post-recall interval
/// reflects the complete history.
public struct LearningScheduler: Scheduler {
    ...
    public func schedule(
        ...
        case .learning:
            ...
            switch rating {
            case .again:
                enterRepairRound(&next, phase: .learning, previousStep: state.stepIndex, now: now)
            case .hard, .good, .easy:
                graduate(&next)
            }
        ...
        case .relearning:
            ...
            switch rating {
            case .again:
                enterRepairRound(&next, phase: .relearning, previousStep: state.stepIndex, now: now)
            case .hard, .good, .easy:
                graduate(&next)
            }
        }
