...
enum StudySupport {
    ...
    static func choiceOptions(for card: DueCard, maximum: Int = 4) -> [String] {
        ...
        let available = card.item.fields
            .flatMap { textRepresentations($0.value) }
        ...
        return deterministicallyReordered(options, seed: card.template.id.uuidString + card.item.id.uuidString)
    }
...
enum AnswerEvaluation: Equatable {
    case correct
    case incorrect
    case unavailable
}
