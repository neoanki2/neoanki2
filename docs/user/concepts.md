...
```text
Item type defines fields and Card setups
                  ↓
Item fills those fields with one piece of knowledge
                  ↓
Each applicable Card setup generates one card
                  ↓
...
starter Card setup turns those fields into one reveal card. If an item type has
two applicable Card setups, one item can generate two cards—for example,
...
**Item type**  
The reusable schema for items: which fields they contain and which Card setups
...
**Card setup**
A data recipe that selects a Question, Answer method, Answer, and static layout.
One applicable Card setup generates one card for an item. The persisted model,
API, and portable formats retain the domain term `Template`.

**Card**  
One retrieval probe generated from an item and a Card setup. A card carries its
...
Store a coherent fact once as an item, then use Card setups for the distinct
...
**Related:** [Item types and Card setups](../item-types-and-templates/) · [Scheduling](../scheduling/)
